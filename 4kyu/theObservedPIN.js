...
variations for an observed PIN with a length of 1 to 8 digits. We could name the function getPINs (get_pins in python).
 But please note that all PINs, the observed one and also the results, must be strings, because of potentially leading
...
function getPINs(observed) {
  ...
  return getPINsRecursive(observed);
}

function getPINsRecursive(observed) {
  if (observed.length === 0) {
    return [''];
  }
  let result = [];
  let comb = getPossibleDigits(parseInt(observed[0]));
  let rest = getPINsRecursive(observed.slice(1));
  for (let j = 0; j < rest.length; j++) {
    for (let i = 0; i < comb.length; i++) {
      result.push(comb[i] + '' + rest[j]);
    }
  }
  return result;
}

function getPossibleDigits(num) {
  switch(num) {
    case 1:
      return [1, 2, 4];
    case 2:
